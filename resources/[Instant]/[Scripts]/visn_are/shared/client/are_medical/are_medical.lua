...
AddEventHandler('esx:onPlayerDeath', function(data)
    ...
    TriggerEvent("visn_phone:playerDied")
...
function _useEmergencyReviveKit(playerId)
    ...
    ESX.TriggerServerCallback('visn_are:getItemCount', function(itemCount)
        if itemCount > 0 then
            createProgressbar(_translate('using_emr'), 120000, 0.10)
        ...
        if IsDisabledControlJustPressed(0, 47) then
            if isUnconscious then
                if cooldownTimer == 0 then
	                TriggerServerEvent("visn_phone:postService", "ambulance", "Bewusstlose Person", GetEntityCoords(PlayerPedId()))
