...
Config.Jail = {
    ...
    BuiltInPrison = {
        ...
        coords = vec3(-546.38, -126.11, 33.75), -- Coords of prison
        spawn = {
            { coords = vec4(-541.69, -119.55, 33.75, 115.55) },
            { coords = vec4(-540.53, -122.91, 33.75, 117.00) },
            { coords = vec4(-539.35, -125.94, 33.75, 113.18) },
            { coords = vec4(-541.25, -130.53, 33.75, 28.28) },
            { coords = vec4(-544.30, -131.76, 33.75, 25.17) },
            { coords = vec4(-547.36, -132.96, 33.75, 21.06) }
        },
        release = vec4(-555.62, -134.43, 38.22, 202.35), -- Coords of released
        jailOutfit = {
            male = {                               -- Male variation
                clothing = {
...
Config.CCTVCameras = { -- CCTV Cameras
    ...
    jobs = {            -- Jobs & minimum ranks that can add/modify/delete CCTV Cameras
        ...
        police = 4,     -- Job 'police' with minimum rank 0
        --        sheriff = 2,    -- Job 'sheriff' with minimum rank 2
    },
    destoryable = false, -- Can players destroy CCTV Cameras?
    blip = {
...
Config.RadarPosts = {           -- Radar posts for speed detection
    ...
    chargeAccount = 'bank',       -- Cash or bank (The default method to charge player
    allowNegativeBalance = true, -- Allow players to go into negative balance? (If false, player must have enough money to pay fine)
...
Config.GSR = {                 -- Gunshot residue settings
    ...
    timeToClean = 1200 * seconds, -- How long to clean GSR in water if enabled
    autoClean = 3600,           -- (IN SECONDS)How long before residue clears itself? Set to false if undesired to auto clean GSR
    command = 'testgsr'            -- Command for testing for GSR? Set to false if undesired
}
...
Config.handcuff = {                             -- Config in regards to cuffing
    ...
    timer = 40 * minutes,                       -- Time before player is automatically unrestrained(Set to false if not desired)
...
Config.Locations = {
    LSPD = {
        blip = {
            enabled = true,
            coords = vec3(-564.40, -126.55, 38.44),
            sprite = 60,
            color = 29,
            scale = 1.0,
            string = 'LAPD Mid-Wilshire Station'
        },
        ...
        clockInAndOut = {
            ...
            coords = vec3(-556.28, -116.91, 42.26), -- Location of where to go on and off duty(If not using target)
        ...
        personalLocker = {
            ...
            coords = vec3(-560.74, -122.83, 42.25), -- Area to prompt personal locker
        ...
        evidenceLocker = {
            ...
            jobLock = 'police',                   -- Job lock?
            coords = vec3(-552.43, -118.86, 33.76), -- Area to prompt personal locker
            range = 2.0,                          -- Range it will prompt from coords above
