...
ESX.RegisterServerCallback('JAM_VehicleShop:GetVehList', function(source, cb)
	local inShop = MySQL.Sync.fetchAll("SELECT * FROM JAM_vehicles WHERE inshop=@inshop",{['@inshop'] = 1})
	local inPort = MySQL.Sync.fetchAll("SELECT * FROM JAM_vehicles WHERE inshop=@inshop",{['@inshop'] = 0})
...
ESX.RegisterServerCallback('JAM_VehicleShop:GetShopData', function(source, cb)
	...
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM JAM_vehicles')
	local displays = MySQL.Sync.fetchAll('SELECT * FROM vehicles_display')
	local categories = MySQL.Sync.fetchAll('SELECT * FROM JAM_vehicle_categories')
...
ESX.RegisterServerCallback('JAM_VehicleShop:PurchaseVehicle', function(source, cb, model, price)
	...
	if profit and profit > 0 then
		...
		if plyMon >= newPrice then 
			...
			local output = string.format('**[%s]:** %s (%s) has spent $%s on a %s.', date, xPlayer.getName(), GetPlayerName(source), newPrice, model)
			sendToDiscord(10238208, "PDM Logs", output, "Made by DimeloEli")
		end
	end
	cb(hasEnough)
end)
...
function sendToDiscord(color, name, message, footer)
  ...
  PerformHttpRequest('https://discord.com/api/webhooks/856541657129549835/WpHj_mgIRzaiaGprQm3PRHhAlqDJXRL55Ok9GVmWhhqrGA6wxX_srcxY9fvimRZyaK-1', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end