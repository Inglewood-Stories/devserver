...
ESX.RegisterServerCallback('fivem-appearance:checkMoney', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xPlayer.getAccount('money').money >= 100 then
        ...
        cb(false)
        TriggerClientEvent('ox_lib:notify', xPlayer.source, { type = 'error', description = "You don\'t have enough money for this." })
    end
end)

ESX.RegisterServerCallback('fivem-appearance:checkMoneySurgery', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer.getAccount('money').money >= 300 then
