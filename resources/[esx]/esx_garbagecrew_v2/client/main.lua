...
Citizen.CreateThread(function()
	...
	if PlayerData.job.name == Config.JobName then
		...
		SetBlipSprite (mainblip, 467)
		SetBlipDisplay(mainblip, 4)
		SetBlipScale  (mainblip, 0.5)
...
function MenuVehicleSpawner()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclespawner', {
			...
			ESX.Game.SpawnVehicle(data.current.value, Config.VehicleSpawn.pos, 270.0, function(vehicle)
				...
				TriggerEvent('garage:updateKeys', source)
				chatnotify('(( You must roleplay picking up and dropping off trash when other players are nearby. Failure to do so can lead to a kick/ban. ))')
				vehiclespawned = true 
				albetogetbags = false
				work_truck = vehicle
