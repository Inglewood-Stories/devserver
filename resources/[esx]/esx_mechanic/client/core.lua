        ...
        if (not uiOpen) then
            if ((playerVeh ~= 0) and (DoesEntityExist(playerVeh)) and (GetPedInVehicleSeat(playerVeh, -1) == playerPed)) then
                ...
                for i = 1, #Config.Positions, 1 do
                    ...
                    if (not tempPos.whitelistJobName or jobName == tempPos.whitelistJobName) then
                        ...
                        local tempActionDist = Config.ActionDistance
                        
                        if (tempDist <= tempActionDist) then
                            waitTime = 0
                            
                            DrawText3Ds(tempPos.pos.x, tempPos.pos.y, tempPos.pos.z, "~b~[E]~s~ To enter customs.")
                            customConfigPosIndex = i
                            if (IsControlJustReleased(0, Config.Keys.action.key)) and customConfigPosIndex ~= 1 then
                                openUI()
                                TriggerServerEvent('getCash', false)
                            elseif (IsControlJustReleased(0, Config.Keys.action.key)) and customConfigPosIndex == 1 then
                                openUI()
                                TriggerServerEvent('getCash2', true)
                            end
            ...
            if (customConfigPosIndex) then
                ...
                if (playerVeh == 0 or playerVeh ~= customVehicle) then
                    ...
                    local tempDist = #(GetEntityCoords(customVehicle) - vector3(tempPos.pos.x, tempPos.pos.y, tempPos.pos.z))
                    if (tempDist > tempActionDist) then
                        closeUI(1, 1)
                    end
                end
...
local societyCash, societyCash2 = nil, nil
RegisterNetEvent('updateCash')
AddEventHandler('updateCash', function(value)
    societyCash = value
end)

RegisterNetEvent('updateCash2')
AddEventHandler('updateCash2', function(value)
    societyCash2 = value
end)
...
function updateCash()
    ...
    if customConfigPosIndex == 1 then
        SendNUIMessage({
            type = 'update',
            what = 'cash',
            cash = societyCash2,
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (ESX) then
                            if customConfigPosIndex ~= 1 then
                                canBuyMod = true
                                if (tempPrice > 0) then
                                    TriggerServerEvent('mechanic:sv:removeCash', tempPrice, false)
                                end
                            elseif customConfigPosIndex == 1 then
                                canBuyMod = true
                                if (tempPrice > 0) then
                                    TriggerServerEvent('mechanic:sv:removeCash', tempPrice, true)
                                end
                            end
                        else
                            canBuyMod = false
