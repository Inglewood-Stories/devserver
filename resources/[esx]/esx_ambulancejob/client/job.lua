		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elseif closestPlayer == -1 or closestDistance > 1.0 then
					lib.notify({ type = 'error', description = _U('no_players') })
				else
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									isBusy = true
									lib.notify({ type = 'inform', description = _U('heal_inprogress') })
									...
									lib.notify({ type = 'success', description = _U('heal_complete', GetPlayerName(closestPlayer)) })
									isBusy = false
								else
									lib.notify({ type = 'inform', description = _U('player_not_conscious') })
								end
							else
								lib.notify({ type = 'error', description = _U('not_enough_bandage') })
							end
						end, 'bandage')
