	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					{label = _U('flat_bed'),  value = 'bc205500w'},
					{label = _U('tow_truck'), value = 'ct660tow'}
				}
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						if
							GetEntityModel(vehicle) == GetHashKey('ct660tow')   or
							GetEntityModel(vehicle) == GetHashKey('bc205500w') or
